...
void MainWindow::onPlay()
{
    ...
    if (pBtn)
    {
        ...
        {
            m_encodeThread = std::thread([this] (){ this->DoEncode();});
            m_encodeThread.join();
...
void MainWindow::DoEncode()
{
    ...
    {
        lame_global_flags * flag = lame_init();
        if (NULL == flag)
        {
            qDebug() << "lame_init fail";
            return;
        }

        FILE * fp = fopen("record.mp3","wb");
        if (NULL == fp)
        {
            qDebug() << "open output file fail";
            return;
        }

        lame_init_params(flag);

        uint8_t* mp3buf = NULL;
        int mp3BufSize = 0;

        std::list<AudioFrame*>::iterator iter = m_audioList.begin();
        for (; iter != m_audioList.end(); iter++)
        {
            AudioFrame* frame = *iter;
            int sample = frame->m_dataLen / 4;
            int size = sample * 1.25 + 7200;
            if (size > mp3BufSize)
            {
                if (mp3buf)
                {
                ...
                mp3buf = new uint8_t[size];
                mp3BufSize = size;
            }
            int encodesize = lame_encode_buffer_interleaved(flag,(short*)frame->m_data,sample,mp3buf,mp3BufSize);
            if (encodesize > 0)
            {
                fwrite(mp3buf,1,encodesize,fp);
            }
        }

        int encodesize = lame_encode_flush(flag,mp3buf,mp3BufSize);
        {
            fwrite(mp3buf,1,encodesize,fp);
        }
