...
wput_socket * socket_new() {
	wput_socket * sock = (wput_socket *)malloc(sizeof(wput_socket));
...
wput_socket *  socket_connect(const unsigned int ip, const unsigned short port){
	...
	*/
	...
	#ifdef WIN32
	{ printf("%d", GetLastError()); return NULL;/*EXIT(4);*/ }
	#else
...
#endif
	res = recv(sock->fd, (char*)buf, len, 0);
...
#endif
	return send(sock->fd, (const char*)buf, len, 0);
}
/* =================================== *
 * ============= utils =============== *
 * =================================== */
...
wput_socket * proxy_init(proxy_settings * ps) {
    ...
    if(t[1] == 2 && ps->user && ps->pass) {
        ...
        char * p = (char*)malloc(strlen(ps->user) + strlen(ps->pass) + 3);
        p[0] = 5;
        p[1] = strlen(ps->user);
        strcpy(p+2, ps->user);
...
wput_socket * proxy_connect(proxy_settings * ps, unsigned int ip, unsigned short port, const char * hostname) {
	...
	if(ps->type == PROXY_SOCKS) {
		...
		if(ip == 0) {
			t = (char*)malloc(6 + strlen(hostname) + 1);
...
{
	HMODULE hLibSSL = LoadLibrary("SSLEAY32.DLL");
